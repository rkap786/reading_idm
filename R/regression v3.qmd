...
ny.adj.mat= getlineardiff(nwea_ny_ability,f, state="NY", start=0.3, end=0.7)
tx.adj.mat= getlineardiff(tx_ability,f, state="Texas", start=0.3, end=0.7)
...
g3= f |> dplyr::select(grade, starts_with("pVal")) |>
  ggplot(aes(grade, pVal.unadj, fill=factor(grade), color=factor(grade))) + geom_boxplot(alpha=0.3) +
  scale_fill_brewer(palette = "RdBu") + 
...
f_all_options=embed.bert |> select(-PassNumUnq, -QNoUnq)
pca_embeddings_80= getpc(f_all_options)
vars.embed.pca= names(pca_embeddings_80)
...
#f.embed.merged.tag= bind_cols(f.embed.merged.tag,pca_embeddings_80)


#f.embed.merged.tag= left_join(f,f.embed.merged.tag)
...
embed.bert.id=embed.bert.id |> select("PassNumUnq", "QNoUnq")
f.embed.merged.tag=bind_cols(embed.bert.id,f.embed.merged.tag)

pca_embeddings_80_tag=bind_cols(embed.bert.id,pca_embeddings_80_tag)
...
f.full= f
f= f[1:1053,] #we dont have embeddings for Texas G3 2022 for now
...
names(embed.llama)= c(paste0("embed.llama", 1:4098))
#embed.bert = embed.bert |> select(-id)

f.with.embed= bind_cols(f,embed.llama)

### Alternate PCA embeddings
pca_embeddings_80= getpc(embed.llama)
vars.embed.pca= names(pca_embeddings_80)
f.with.embed= bind_cols(f.with.embed,pca_embeddings_80)
...
```{r}
res_unadj_prev2=res_unadj
res_cons_prev2=res_cons
res_irt_prev2= res_irt
...
f.input= f.with.embed
f.input$pVal= f.input$pVal.unadj
vars_select=c("pVal", c(vars.chars,vars.lang))


#Models
res_item= fnc_reg_elasticNet(f.input,
                             c("pVal", c(vars.fe,vars.chars))) # without language metrics
res_lang= fnc_reg_elasticNet(f.input, 
                              c("pVal",c(vars.fe,vars.lang))) ## everything except embedding
res_item_embed= fnc_reg_elasticNet(f.input, 
                              c("pVal",c(vars.fe,vars.embed))) ## embeddings only

res_item_embed_pca= fnc_reg_elasticNet(f.input, 
                              c("pVal",c(vars.fe,vars.embed.pca))) ## embeddings only

res_item_all= fnc_reg_elasticNet(f.input, 
                              c("pVal",c(vars.fe,vars.chars,
                                         vars.lang, vars.embed)))
...
res_unadj= bind_rows(res_item, res_lang, res_item_embed, res_item_embed_pca, 
                     res_item_all, res_item_all_pca)

res_unadj= round(res_unadj,2)
res_unadj= bind_cols(type=rep("Unadjusted",nrow(res_unadj)),model=c("Only assessment characteristics", "Only text analysis metrics", 
"Only LlAMA embeddings", 
"Only PCA LlAMA embeddings", 
...
res_irt= bind_rows(res_item, res_lang, res_item_embed, res_item_embed_pca, 
                     res_item_all, res_item_all_pca)

res_irt= round(res_irt,2)
res_irt= bind_cols(type=rep("IRT",nrow(res_irt)),model=c("Only assessment characteristics", "Only text analysis metrics", 
"Only LlAMA embeddings", 
"Only PCA LlAMA embeddings", 
"Assessment characteristics, text analysis metrics, & LlAMA embeddings", 
"Assessment characteristics, text analysis metrics, & PCA on LlAMA embeddings"), res_irt)
...
res_cons= bind_rows(res_item, res_lang, res_item_embed, res_item_embed_pca, 
                     res_item_all, res_item_all_pca)

res_cons= round(res_cons,2)
res_cons= bind_cols(type=rep("Cons",nrow(res_cons)),model=c("Only assessment characteristics", "Only text analysis metrics", 
"Only LlAMA embeddings", 
"Only PCA LlAMA embeddings", 
"Assessment characteristics, text analysis metrics, & LlAMA embeddings", 
"Assessment characteristics, text analysis metrics, & PCA on LlAMA embeddings"), res_cons)
